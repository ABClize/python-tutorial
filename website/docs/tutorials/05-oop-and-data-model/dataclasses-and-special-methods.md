...
需要初始化后处理时可以定义 `__post_init__()`。下面生成一个用于降序排列分数的辅助字段。它不参与
初始化参数，也不出现在自动生成的显示结果中：
...
`order=True` 根据字段顺序生成比较方法。辅助字段 `sort_index` 放在第一位，因此控制主要排序规则。
`init=False` 表示它不作为初始化参数，`repr=False` 表示它不出现在自动生成的 `repr` 中；这个字段
仍然可以通过实例访问，并不是私有字段。
...
特殊方法应符合用户熟悉的行为。例如 `__len__()` 必须返回非负整数，索引越界应抛出
`IndexError`。
...
定义值相等时，应同时考虑对象是否可变，以及对象是否需要放进 set 或作为 dict 的键。下面让坐标按
`x` 和 `y` 比较：
