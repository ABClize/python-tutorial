...
变量保存的不是一个可以随意复制的盒子，而是对对象的引用。先理解变量、对象和引用的关系，才能解释为什么修改列表会影响另一个变量，而重新赋值却不会。本页从对象身份讲起，再区分可变对象与不可变对象。
...
`copied = original` 复制了对象引用，没有创建新的列表。
...
判断一个操作会不会影响其他变量时，需要先确认它是在修改对象，还是只在改变某个变量的指向。
...
元组仍然保存原来的两个元素引用。被修改的是第二个元素所指向的列表，不是元组结构。
