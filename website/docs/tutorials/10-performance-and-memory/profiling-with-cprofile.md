# 使用 cProfile 找出最耗时的函数

`cProfile` 用于统计 Python 函数的调用次数和耗时。程序包含多层函数调用时，它可以指出时间主要花在
哪个函数中，而不只是告诉我们“整体很慢”。程序中占用大量时间、值得优先检查的部分，称为性能热点。
...
`cProfile` 属于确定性性能分析器：它会在每次函数调用和返回时记录事件，因此会增加额外开销。
`cProfile` 输出中的总时间不能直接当作未开启性能分析时的真实生产耗时。
