...
“程序很慢”不是一个可以直接测量的指标。它可能指单次请求延迟高、批处理吞吐低、输入变大后耗时
急剧上升，也可能只是某个外部接口偶尔超时。测量之前，要先把现象改写成明确的问题。
...
`timeit` 会重复执行被测代码，适合范围明确的微基准。它比手写一次
`time.perf_counter()` 更容易降低偶发系统调度带来的噪声。
