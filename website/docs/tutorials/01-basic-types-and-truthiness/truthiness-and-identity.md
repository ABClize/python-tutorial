...
if 判断的并不总是字面上的 True 或 False，任何对象都可以按真值规则参与条件判断。同时，值相等和两个变量指向同一个对象是两回事。本页用可操作示例区分 bool()、== 和 is。
...
`"0"` 是包含一个字符的非空字符串，所以为真；`[0]` 是包含一个元素的非空列表，也为真。Python 不会
根据字符串或容器内部的内容猜测真假。
...
`==` 比较两个对象的值是否相等，`is` 比较两个表达式是否得到同一个对象。
