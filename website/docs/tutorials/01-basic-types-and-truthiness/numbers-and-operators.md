...
数字运算不只是加减乘除：比较会得到布尔值，逻辑运算还会决定表达式是否继续计算。本页把这些规则放在一起，并解释浮点数误差、布尔值与整数的关系，以及 Python 整数为什么不容易溢出。
...
左边已经是 `False`，右边的 `10 / value` 没有执行，因此不会发生除零错误。
...
许多十进制小数无法用有限的二进制位精确表示。近似数值比较可以使用 `math.isclose()`：
