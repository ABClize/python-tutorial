...
`match` 根据值的结构选择分支。Python 会从上到下尝试各个 `case`，只执行第一个匹配成功并且 guard
成立的分支。
...
不带 guard 的 `_` 会匹配任何值，所以必须放在最后。

### 序列模式和 guard

序列模式可以像解包一样匹配 list 或 tuple，并把元素绑定到变量。guard 是写在模式后面的
`if` 条件，模式和 guard 都成立时才执行该分支：
...
比较固定值时应直接写 `case "ready":`。未加 guard 的捕获模式总能匹配，也必须放在最后；
