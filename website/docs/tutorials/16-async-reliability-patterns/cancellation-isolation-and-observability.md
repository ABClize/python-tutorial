...
异步任务可能因为客户端断开、外层超时、兄弟任务失败或应用关闭而被取消。可靠代码不仅要停止工作，
还要释放资源、限制故障影响范围，并留下足够信息判断策略是否有效。
...
锁、响应流、临时文件和数据库事务都需要类似结构。若清理本身也可能长时间等待，还要设计独立但有限的
收尾预算。
...
如果商品查询和风控共用全部连接与 Semaphore，风控流量突增可能拖垮商品接口。为不同依赖分配独立预算，
就是隔舱思想。
...
熔断需要定义失败窗口、阈值、冷却、探测和多实例一致性。当前仓库没有实现熔断器或跨业务隔舱，这些是
扩展设计，不应被当作现有功能。
...
平均延迟会掩盖长尾。容量和 timeout 通常要结合 p95、p99、超时率和错误率观察。
