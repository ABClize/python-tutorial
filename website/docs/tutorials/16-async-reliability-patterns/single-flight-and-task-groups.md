...
合并相同键请求（single-flight）是把相同键的并发请求合并成一次实际加载，其余请求共享结果。
请求竞速是并发调用多个结果等价的来源，接受第一个成功结果，再取消并等待其他调用收尾。
`TaskGroup` 适合组织一组需要共同成功的子任务。
...
普通缓存到期时，许多请求可能同时发现未命中，并一起涌向下游。这种现象常称为“惊群”：
...
是否要缓存“未找到”等失败结果（负缓存）、删除不再使用的锁，或者让多个进程共同协调加载，要根据值
类型、key 数量和部署方式决定。

## 多个等价来源竞速成功结果

当几个来源返回结果的含义相同，可以同时发起请求，并采用第一个成功结果。仓库的 `first_success()`：
...
请求竞速会增加下游流量，只适合少量、结果确实等价且容量彼此独立的来源。
...
如果商品和库存查询只采用第一个成功结果，那么只查询到其中一项也会继续，业务结果明显错误。若把
镜像来源放进 TaskGroup，一个快速失败会取消另一个可能成功的来源。
...
TaskGroup 解决“这些子任务由谁管理、何时结束”，容量限制解决“同时允许多少任务”。两个问题都要明确。
