...
一段代码对几个示例返回了正确结果，并不能证明它对所有合法输入都正确。理解算法时，可以关注每轮
循环结束后始终成立的事实，这种事实称为循环不变量。

<p class="source-note">对应源码：<code>python/python_interview_practice/08_algorithms.py</code></p>

## 两数之和：保存已经见过的信息

给定整数列表和目标值，找出两个数，使它们的和等于目标值，并返回这两个数的下标。
...
查找 `2` 时，返回 1、2 或 3 都符合原契约。如果需求是“返回第一个 2”，找到目标后不能立即返回，
而要继续向左收缩范围：
...
算法是否正确取决于契约。把“找到任意一个”和“找到第一个”写清楚，比记住某段模板更重要。
