...
双指针、计数表，或者先把数据转成统一形式，再将结果作为字典键来减少重复工作。
...
这个函数只展平一层。输入 `[[1, [2]]]` 时，内层列表 `[2]` 仍会保留。如果要求递归展开所有层级，
还要规定字符串、元组等对象是否也应被展开。

## 把数据转成统一形式后分组

字母异位词包含相同字母，只是顺序不同。把每个单词的字符排序后，可得到同组单词共有的键。这种先把
等价输入转成统一形式的处理称为规范化：
