...
分层是把不同职责放到不同模块。API 层处理 HTTP，请求与响应数据模型处理输入输出结构，服务层组织
业务步骤，领域对象保存业务规则，仓储负责数据存取，网关负责调用外部系统。项目文件名是
`schemas.py`，但这里的数据模型不要和 OpenAPI 使用的 JSON Schema 混淆。
...
这里的业务用例（use case）是一次可独立完成的业务流程，不是测试用例。
...
当前仓库提供内存仓储和返回固定、可预测结果的 Fake 网关，所以示例无需数据库或真实远程服务就能
运行。以后替换成 PostgreSQL 或 HTTP Client 时，服务层的业务流程不需要随之重写。
...
| 请求与响应模型 | 外部数据的类型、格式和局部关系 |
| Service | 一个业务用例要按什么顺序协调哪些能力 |
| Domain | 业务状态、计算和合法转换 |
| Repository / Gateway | 数据库与外部系统的交互 |

如果两个文件只是转发参数，没有各自独立的职责，继续拆分不会自动让架构更好。分层应减少耦合，而
不是追求目录形式。
