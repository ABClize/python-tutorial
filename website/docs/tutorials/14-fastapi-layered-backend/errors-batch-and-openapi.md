...
| `UpstreamTimeoutError` | 504 | 商品、库存等外部依赖未按时完成 |
...
400、409 与 422 的区别不是绝对语法规则，而是接口需要提前说明的行为。本项目用 422 表达输入结构
校验失败，用 409 表达请求结构正确但与当前业务状态冲突。
...
仓储的列表读取和总数查询互不依赖，服务层用 `asyncio.gather()` 并发等待。真实数据库至少要使用
稳定且唯一的排序。若还要避免翻页期间插入或删除数据造成重复、遗漏，应使用游标分页、快照或一致性
读取；仅有稳定排序并不足以解决数据变化问题。
...
这能控制一次批量请求内部同时执行的任务数量，但当前依赖会为每个 HTTP 请求创建一个
`OrderService`，所以 Semaphore 不是全进程限流器。两个批量请求仍各自拥有一份上限。
...
本项目要求 TaskGroup 中的兄弟任务共同成功。业务要求不同，响应模型和持久化方式也要一起改变。
...
合适的安全依赖，并检查生成的文档是否正确说明认证方式。

## 分层应用的检查清单

- 路由只处理 HTTP 输入和输出，不承载完整业务流程；
