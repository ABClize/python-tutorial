...
单元测试检查一个函数或类。集成测试检查多个组件能否配合。API 测试通过 HTTP 调用接口。基于性质的
测试会生成多组输入，检查某条规则是否一直成立。不同测试层次解决不同问题。
...
| API 测试 | HTTP 请求、校验和响应 | 测试应用与客户端 | 验证接口约定的输入、输出和错误行为 |
...
API 测试还应检查错误结构、认证、请求头，同一个请求重复发送时是否会重复创建数据，以及能否用测试
替身代替数据库等真实依赖。如果应用有需要随程序启动和关闭的资源，也就是由 lifespan 管理的资源，
应把 `TestClient` 放进上下文管理器。这样启动和关闭代码都会执行：
...
基于性质的测试不替代有名字的业务样例。错误消息、权限和关键边界仍适合用明确示例表达。
...
- 公开输入输出；
- 边界和失败分支；
- 金额、权限和“重复请求不能重复产生结果”（幂等性）等高风险规则；
- 是否执行了写数据库、发送消息等外部操作，以及使用的参数是否正确；
- 过去出现过的缺陷；
- 超时、取消和资源清理；
- 模块之间约定的输入、输出和错误行为。
